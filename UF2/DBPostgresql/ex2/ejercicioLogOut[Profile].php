<?php
    session_start();
    //PARA DISPONER DE LA CLASS CONEXION
    include 'conexion/conexion.php';
    $conexion = new Conexion();
    //COMPROBAMOS EL ESTADO DE LA CONEXION , SI SE HA ESTABLECIDO CON EXITO
    if (strcmp($conexion->statusConnection(),'Conectado')==0) {
        ...
        if (isset($_POST['profile'])) {
            ...
            if (isset($_POST['RememberMe'])) { setcookie("Username",$username,time()+3600); }
                //VALIDACIÓN DE DATOS , MEDIANTE UNA CONSULTA A LA BASE DE DATOS
                if ( !($conexion->checkUser($username,$password))) {
    ...
    //ERROR AL CONECTAR CON LA BASE DE DATOS
        ...
        setcookie("Error"," No se ha establecido conexion a la base de datos !",time()+3600);
        header("location: ejercicioLogOut[LogIn].php");
    }
